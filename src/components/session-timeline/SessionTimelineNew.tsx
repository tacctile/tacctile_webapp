...
const UserSectionHeader = styled(Box)<{ color?: string }>(({ color = '#19abb5' }) => ({
  display: 'flex',
  alignItems: 'center',
  gap: 8,
  padding: '8px 12px',
    ...
    return (
      <Container>
        <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Typography sx={{ color: '#555' }}>No evidence in this session. Import files to begin.</Typography>
        </Box>
      </Container>
    );
  }
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <PreviewDropdown size="small">
                  <Select
                    ...
                    onChange={(e) => setSelectedVideoUser(e.target.value)}
                    displayEmpty
          ...
          <ImagePreviewPane>
                  ...
                  {allImages.length === 0 ? (
                    ...
                    allImages.map((img) => (
                      <MenuItem key={img.id} value={img.id}>
                        {img.user}: {img.fileName.slice(0, 20)}...
                      </MenuItem>
                    ))
                  )}
                </Select>
            ...
            <PreviewContent>
              {selectedImage ? (
                ...
                <Typography sx={{ color: '#444', fontSize: 12 }}>Select an image</Typography>
              )}
            </PreviewContent>
          </ImagePreviewPane>
        </PreviewRow>
        ...
        <AudioPreviewPane>
          ...
          <PreviewDropdown size="small">
            <Select
              ...
              onChange={(e) => setSelectedAudioUser(e.target.value)}
              displayEmpty
        ...
        <LanesContainer ref={lanesRef} onClick={handleTimelineClick}>
          {userSections.map((section) => (
            <UserSectionContainer key={section.userName}>
              <UserSectionHeader
                color={getUserColor(section.userName)}
              ...
              >
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  <Typography sx={{ fontSize: 10, color: '#c45c5c' }}>
                    {section.videos.length} vid
                  </Typography>
                  <Typography sx={{ fontSize: 10, color: '#5a9a6b' }}>
                    {section.audios.length} aud
                  </Typography>
